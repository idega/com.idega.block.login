...
/**
 * Login component which is using TWO step auth authentication
 * @author zygimantas
 *
 */
public class LoginWithTwoStepAuth extends Login2 {

	@Autowired
	private JQuery jQuery;


	public LoginWithTwoStepAuth() {
		super();
	}
	...
	public void initializeComponent(FacesContext context) {
		...
		if (getTwoStepAuthenticationFaceletPath() == null) {
			setTwoStepAuthenticationFaceletPath(getBundle(context, getBundleIdentifier()).getFaceletURI("twoStepAuthLogin.xhtml"));
		}
		...
		String action = "LoginHelper.errorMessage = '" + iwrb.getLocalizedString("login.error_logging_in", "Error logging in: make sure user name, password and second step auth code are entered!") +
		...
		if (iwc.isLoggedOn()) {
			...
			if (state.equals(LoginState.LOGGED_OUT) || state.equals(LoginState.NO_STATE)) {
				...
				add(getLoginWithTwoStepAuthCodePart(context, bean));
			} else {
	...
	protected UIComponent getLoggedOutPart(FacesContext context, LoginBean bean) {
		...
		bean.addParameter(LoginBusinessBean.LoginStateParameter, LoginBusinessBean.LOGIN_EVENT_2_STEP_AUTH_LOGIN);
	...
	protected UIComponent getLoginWithTwoStepAuthCodePart(FacesContext context, LoginBean bean) {
		IWContext iwc = IWContext.getIWContext(context);

		boolean hiddenParamAdded = false;
		bean.setAllowCookieLogin(getAllowCookieLogin());
		bean.setAction(iwc.getRequestURI(getSendToHTTPS()));
		bean.addParameter(LoginBusinessBean.LoginStateParameter, LoginBusinessBean.LOGIN_EVENT_FULL_WITH_2_STEP_AUTH_LOGIN);
		if (getRedirectUserToPrimaryGroupHomePage()) {
			bean.addParameter(IWAuthenticator.PARAMETER_REDIRECT_USER_TO_PRIMARY_GROUP_HOME_PAGE, Boolean.TRUE.toString());
		} else if (getURLToRedirectToOnLogon() != null) {
			bean.addParameter(IWAuthenticator.PARAMETER_REDIRECT_URI_ONLOGON, getURLToRedirectToOnLogon());
		...
		String email = (String) iwc.getRequest().getAttribute(LoginBusinessBean.PARAMETER_EMAIL);
		if (!StringUtil.isEmpty(email)) {
			email += CoreConstants.DOT;
			bean.addParameter(LoginBusinessBean.PARAMETER_EMAIL, email);
		}
		String twoStepAuthCodeValidity = (String) iwc.getRequest().getAttribute(LoginBusinessBean.PARAMETER_2_STEP_AUTH_CODE_VALIDITY);
		if (!StringUtil.isEmpty(twoStepAuthCodeValidity)) {
			IWResourceBundle iwrb = getBundle(context, getBundleIdentifier()).getResourceBundle(iwc);
			Double minutes = Math.floor(Double.valueOf(twoStepAuthCodeValidity) / 60);
			Double seconds = Double.valueOf(twoStepAuthCodeValidity) % 60;
			String twoStepAuthCodeValidityStr = CoreConstants.EMPTY;
			if (minutes != null && minutes > 0) {
				Integer minutesInt = minutes.intValue();
				if (minutesInt != null) {
					twoStepAuthCodeValidityStr += minutesInt;
					twoStepAuthCodeValidityStr += CoreConstants.SPACE;
					twoStepAuthCodeValidityStr += iwrb.getLocalizedString("minutes.short", "min");
				}
			}
			if (seconds != null && seconds > 0) {
				...
				if (secondsInt != null) {
					twoStepAuthCodeValidityStr += CoreConstants.SPACE;
					twoStepAuthCodeValidityStr += secondsInt;
					twoStepAuthCodeValidityStr += CoreConstants.SPACE;
					twoStepAuthCodeValidityStr += iwrb.getLocalizedString("seconds.short", "sec");
				}
			}
			twoStepAuthCodeValidityStr += CoreConstants.DOT;
			bean.addParameter(LoginBusinessBean.PARAMETER_2_STEP_AUTH_CODE_VALIDITY, twoStepAuthCodeValidityStr);
		}
		...
		FaceletComponent facelet = (FaceletComponent) iwc.getApplication().createComponent(FaceletComponent.COMPONENT_TYPE);
		facelet.setFaceletURI(getTwoStepAuthenticationFaceletPath());
