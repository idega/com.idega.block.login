...
public class PasswordChangerBean {
	...
	public String getToken() {
		String newToken = getIWContext().getParameter(PasswordTokenCreator.PARAMETER_TOKEN);
	...
	public void submit(IWContext iwc) {
		...
		User user = getPasswordTokenBusiness().completePasswordReset(iwc, token, newPassword);
		...
		if(changed && redirectToHomepage){
			try {
				...
				iwc.getExternalContext().redirect(url);
			} catch (Exception e) {
